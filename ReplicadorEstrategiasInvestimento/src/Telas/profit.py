...
from ctypes import CFUNCTYPE,c_int,c_uint, c_int32,c_double, c_wchar_p, WinDLL,Structure,POINTER,c_longlong
from dataclasses import dataclass
from ctypes import CFUNCTYPE



profit_dll = WinDLL(r'C:\Users\felip\OneDrive\Área de Trabalho\ProfitDLL\DLL e Executável\ProfitDLL64.dll'
)
...
@CFUNCTYPE(None, TAssetID, c_wchar_p, c_uint, c_double, c_double, c_int, c_int, c_int, c_int)
...
@CFUNCTYPE(None, TAssetID, c_int)
def progressCallBack(assetId, nProgress):
    print(assetId.ticker + ' | Progress | ' + str(nProgress))
    return


@CFUNCTYPE(None, c_int, c_wchar_p, c_wchar_p, c_wchar_p)
def accountCallback(nCorretora, corretoraNomeCompleto, accountID, nomeTitular):
    print("Conta | " + accountID + ' - ' + nomeTitular + ' | Corretora ' + str(
        nCorretora) + ' - ' + corretoraNomeCompleto)
    return


@CFUNCTYPE(None, TAssetID, c_int, c_int, c_int, c_int, c_int, c_double, POINTER(c_int), POINTER(c_int))
...
@CFUNCTYPE(None, TAssetID, c_wchar_p, c_uint, c_double, c_double, c_int, c_int, c_int, c_int, c_wchar_p)
...
@CFUNCTYPE(None, TAssetID, c_double, c_int, c_int)
...
@CFUNCTYPE(None, TAssetID, c_wchar_p, c_double, c_double, c_double, c_double, c_double, c_double, c_double, c_double,
             c_double,
...
def newDailyCallback(assetID, date, sOpen, sHigh, sLow, sClose, sVol, sAjuste, sMaxLimit, sMinLimit, sVolBuyer,
    ...
    print(
        assetID.ticker + ' | DailySignal | ' + date + ' Open: ' + str(sOpen) + ' High: ' + str(sHigh) + ' Low: ' + str(
            sLow) + ' Close: ' + str(sClose))
    return
...
@CFUNCTYPE(None, TAssetID, c_int, c_int, c_int, c_int, c_int, c_longlong, c_double, c_int, c_int, c_int, c_int, c_int,
...
@CFUNCTYPE(None, TAssetID, c_wchar_p, c_wchar_p, c_int, c_int, c_int, c_int, c_int, c_double, c_double, c_wchar_p,
             c_wchar_p)
def assetListInfoCallback(assetId, strName, strDescription, iMinOrdQtd, iMaxOrdQtd, iLote, iSecurityType,
                          iSecuritySubType, dMinPriceInc, dContractMult, strValidDate, strISIN):
...
def assetListInfoCallbackV2(assetId, strName, strDescription, iMinOrdQtd, iMaxOrdQtd, iLote, iSecurityType,
                            iSecuritySubType, dMinPriceInc, dContractMult, strValidDate, strISIN, strSetor, strSubSetor,
                            strSegmento):
...
def historyCallbackV2(rAssetID,
                      ...
                      nProfitID,
                      tipoOrdem, conta, titular, clOrdID, status, creationDate, lastUpdateDate, closeDate,
                      validityDate):
    global gOrders
    global rotPassword
...
def getAllTickersByStock():
    ...
    if (stock == ''):
        res = profit_dll.GetAllTicker('')
        res = profit_dll.GetAllTicker(stock)
...
def printPosition():
    ...
    result = profit_dll.GetPosition(c_wchar_p(str(acc_id)), c_wchar_p(str(corretora)), c_wchar_p(asset),
                                    c_wchar_p(bolsa))
    ...
    if (n_qtd == 0):
        ...
        for i in range(n_qtd):
            ...
            print(
                f"Corretora: {corretora_id}, Titular: {str(titular)}, Ticker: {str(ticker)}, Price: {price}, AvgSellPrice: {avg_sell_price}, AvgBuyPrice: {avg_buy_price}, SellQtd: {sell_qtd}, BuyQtd: {buy_qtd}")


def dllStart(chave_acesso, usuario, senha):
    try:
        global profit_dll
        ...
        if bRoteamento:
            result = profit_dll.DLLInitializeLogin(c_wchar_p(chave_acesso), c_wchar_p(usuario), c_wchar_p(senha), stateCallback,
                                                   historyCallBack, orderChangeCallBack, accountCallback,
                                                   newTradeCallback, newDailyCallback, priceBookCallback,
                                                   offerBookCallback, newHistoryCallback, progressCallBack,
                                                   newTinyBookCallBack)
        else:
            result = profit_dll.DLLInitializeMarketLogin(c_wchar_p(chave_acesso), c_wchar_p(usuario), c_wchar_p(senha),
                                                         stateCallback, newTradeCallback, newDailyCallback,
                                                         priceBookCallback,
                                                         offerBookCallback, newHistoryCallback, progressCallBack,
                                                         newTinyBookCallBack)
...
def selectOrder():
    ...
    if len(gOrders.keys()) > 0:
        ...
        for key, value in gOrders.items():
            print('{0}. ProfitID={1} ClordID={2}, price={3}, amount={4}, status={5}'.format(i, value.profitid,
                                                                                            value.clordid, value.price,
...
def changeOrder():
    ...
    profit_dll.SendChangeOrder(c_wchar_p(accountID), c_wchar_p(corretora), c_wchar_p(rotPassword), order.clordid,
                               c_double(price), c_wchar_p(amount))
